...
function save_form_settings($post_id) {
    ...
    if (isset($_POST['fields_config'])) {
        $fields_config = sanitize_textarea_field($_POST['fields_config']);
...
class FormsHandler {
    ...
    public function register_rest_routes() {
        register_rest_route('wp/v2', '/forms/(?P<slug>[a-zA-Z0-9-]+)/submit', array(
            'methods' => 'POST',
    ...
    private function process_form_submission($form_id, $form_data) {
        ...
        $delivery_results = array(
            'email' => array('success' => false, 'error' => null),
            'telegram' => array('success' => false, 'error' => null, 'enabled' => false),
            'google_sheets' => array('success' => false, 'error' => null, 'enabled' => false),
            'admin_storage' => array('success' => false, 'error' => null, 'enabled' => false)
        );
        ...
        if (!$email_success && !$telegram_success && !$sheets_success) {
            return new WP_Error('send_error', 'Failed to send email, Telegram notification, and Google Sheets data', array(
                'status' => 500,
    ...
    private function validate_form_data($form_data, $fields_config) {
        ...
        foreach ($fields as $field) {
            ...
            if ($is_required) {
                if (!isset($form_data[$field_name]) || $form_data[$field_name] === '' || $form_data[$field_name] === null) {
...
function fill_forms_admin_columns($column, $post_id) {
    switch ($column) {
        ...
        case 'admin_storage':
            ...
            break;
        case 'export':
            $submissions_count = get_posts(array(
                'post_type' => 'form_submissions',
                'post_status' => 'publish',
                'posts_per_page' => -1,
                'meta_query' => array(
