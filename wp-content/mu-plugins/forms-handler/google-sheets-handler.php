...
function save_google_sheets_settings() {
    ...
    // Обработка полной очистки настроек
...
class GoogleSheetsHandler {
    ...
    public static function send_data($spreadsheet_id, $sheet_name, $form_data, $form_title) {
        ...
        $response = wp_remote_post("https://sheets.googleapis.com/v4/spreadsheets/{$spreadsheet_id}/values/{$range}:append?valueInputOption=RAW", array(
    ...
    public static function validate_config($spreadsheet_id, $sheet_name = '') {
        ...
        $response = wp_remote_get("https://sheets.googleapis.com/v4/spreadsheets/{$spreadsheet_id}", array(
    ...
    public static function test_connection($spreadsheet_id) {
        $results = array(
            'authentication' => array('valid' => false, 'error' => ''),
            'spreadsheet' => array('accessible' => false, 'error' => '')
        );
